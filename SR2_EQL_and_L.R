# Program for design parameter (L) and EQL value of X_bar chart with SR2 scheme
...
f1=function(del) {
  ...
  p5=pnorm(qnorm(alp/2,0,1)/2-del*sqrt(n),0,1)-pnorm(qnorm(alp/2,0,1)-del*sqrt(n),0,1)
  p6=pnorm(qnorm(alp/2,0,1)-del*sqrt(n),0,1)
  (1/(del_max-del_min))*((del^2)*(-((p1 + 1)*(p6 + 1))/(p2 + p3 + p4 + p5 + p1*p2 + p1*p3 + p1*p4 + p1*p5 + p1*p6 + p2*p6 + p3*p6 + p4*p6 + p5*p6 + p1*p2*p6 + p1*p3*p6 + p1*p4*p6 + p1*p5*p6 - 1)
  ))
}
